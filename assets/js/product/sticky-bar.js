...
function handleVariantChange(event) {
  // The sticky CTA mirrors the MAIN product. Ignore variant changes that came
  // from a quick-view popover (root is the modal, not the document), otherwise
  // previewing a recommended product would rewrite this page's sticky price.
  ...
  // Original (before) price — shown struck-through only when the variant is on sale.
  if (stickyPriceOldEl) {
    if (selectedProduct.formatted_sale_price) {
      stickyPriceOldEl.textContent = selectedProduct.formatted_price;
      stickyPriceOldEl.classList.remove("hidden");
    } else {
      stickyPriceOldEl.classList.add("hidden");
    }
  }
...
export function init() {
  ...
  stickyPriceOldEl = document.querySelector("[data-sticky-cta-price-old]");
  ...
  // The floating CTA is ALWAYS visible now (no scroll-triggered show/hide).
  // It ships without the `hidden` class, so there's no IntersectionObserver
  // here anymore — we only keep the variant/stock/add-to-cart wiring below.
