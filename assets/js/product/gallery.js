...
export function initProductGallery(galleryId) {
  ...
  // Thumbnails are a static vertical column beside the main image (no
  // carousel needed) — click + active-highlight are wired below.
  ...
  const updateActiveThumb = () => {
    ...
    thumbButtons.forEach((btn, index) => {
      if (index === selectedIndex) {
        btn.classList.add("border-[#eee]");
        btn.classList.remove("border-transparent");
      } else {
        btn.classList.remove("border-[#eee]");
