 ...
 *
 * Standalone script for loyalty rewards popup on non-product/cart pages.
 * This script runs AFTER vitrin_body and uses global variables set in layout.jinja.
 *
 ...
 * - text_loyalty_options: Translated template like "For {points} get {percentage}"
 * - store_currency_code: Currency code like "SAR"
 */
...
window.addEventListener("load", function () {
  ...
  if (typeof store_currency_code === "undefined") {
    return;
  }
...
// Create floating button (hidden initially until rewards load)
...
// Fetch redemption methods and populate popup
function getRedemptionMethods() {
  if (!window.zid || !window.zid.cart || !window.zid.cart.getRedemptionMethods) {
    return;
  }

  zid.cart
    .getRedemptionMethods(store_currency_code, { showErrorNotification: true })
    .then(function (response) {
      if (response && response.options) {
        redemptionMethods = response.options;

        if (redemptionMethods.length > 0) {
          const container = document.querySelector(".loyalty-rewards-popup__rewards");

          if (container) {
            redemptionMethods.forEach(function (method) {
              if (method.is_active) {
                const row = document.createElement("div");
                row.className = "loyalty-rewards-popup__rewards_row";

                const text = text_loyalty_options
                  .replace("{points}", method.points_to_redeem)
                  .replace("{percentage}", method.reward.discount_value.toFixed(2) + " " + store_currency_code);
            ...
            // Show button after rewards are populated
            showFloatingButton();
          } else {
            // Container not found, still show button
            showFloatingButton();
          }
        } else {
          // No active methods, show button for "More" link
          showFloatingButton();
        }
      } else {
        // API returned but no options
        showFloatingButton();
      }
    })
    .catch(function () {
      // Show button even on error
      showFloatingButton();
    });
}

// Get customer loyalty points
function getCustomerLoyaltyPoints() {
  if (!window.zid || !window.zid.cart || !window.zid.cart.getCustomerLoyaltyPoints) {
    return;
  }

  zid.cart.getCustomerLoyaltyPoints({ showErrorNotification: true }).then(function (response) {
    if (response && response.status === "success" && response.data && response.data.points) {
      customerPoints = response.data.points.available_poitns || response.data.points.available_points || 0;
      const el = document.getElementById("customerPointsValue");
      if (el) {
        el.textContent = customerPoints;
      }
      const info = document.querySelector(".loyalty-points__info");
      if (info) {
        info.classList.remove("loyalty-points__info_d_none");
      }
    }
  });
}
...
window.popupLoyaltyReward = function () {
  ...
  if (popupInit) {
    ...
    popup.classList.remove("loyalty-rewards-popup-hidden");
  }
};
