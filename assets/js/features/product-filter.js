...
class ProductFilter {
  ...
  /**
   * Load More — fetch the NEXT page and APPEND its product cards to the grid
   * (instead of numbered pagination). Wired from the load-more button's
   * onclick. The button carries the page bookkeeping in data attributes:
   *   data-load-more-next  — the page number to fetch on the next click
   *   data-load-more-total — total page count (button retires past it)
   */
  async loadMore(button) {
    if (this.isLoading || !button) return;

    const nextPage = parseInt(button.dataset.loadMoreNext || "0", 10);
    const totalPages = parseInt(button.dataset.loadMoreTotal || "0", 10);
    if (!nextPage || (totalPages && nextPage > totalPages)) return;

    const grid = document.querySelector(`${this.contentSelector} .products-grid`);
    if (!grid) return;
    ...
    // Build the next-page URL from the current location so active filters,
    // sort and page_size are preserved.
    const url = new URL(window.location.href);
    ...
    try {
      ...
      const newGrid = doc.querySelector(`${this.contentSelector} .products-grid`);

      if (newGrid) {
        const frag = document.createDocumentFragment();
        Array.from(newGrid.children).forEach((card) => frag.appendChild(card));
        grid.appendChild(frag);

        // Re-bind interactive bits on the freshly appended cards:
        //   products:updated  → cart + quick-view buttons, card ratings
        //   products-updated  → wishlist hearts, qty steppers
        window.dispatchEvent(new CustomEvent("products:updated"));
        window.dispatchEvent(
          new CustomEvent("products-updated", { detail: { container: grid } })
        );
      }

      // Refresh the "X / Y" shown-count, if present.
      const root = button.closest("[data-load-more-root]");
      const countEl = root && root.querySelector("[data-load-more-count]");
      if (root && countEl) {
        const total = parseInt(root.dataset.loadMoreTotalCount || "0", 10);
        countEl.textContent = `${grid.children.length} / ${total}`;
      }

      // Advance the button, or retire it once the last page has loaded.
      if (!totalPages || nextPage >= totalPages) {
        button.remove();
      } else {
        button.dataset.loadMoreNext = String(nextPage + 1);
      }
    } catch (error) {
      console.error("ProductFilter: Load more error", error);
    } finally {
      this.isLoading = false;
