...
export function applyCoupon(onSuccess) {
  ...
  window.zid.cart
    .applyCoupon({ coupon_code: input.value.trim() }, { showErrorNotification: false })
    .then(() => {
      if (onSuccess) onSuccess();
    })
    .catch((err) => {
      cartErrorToast(err, "Couldn't apply the coupon");
...
export function removeCoupon(onSuccess) {
  ...
  window.zid.cart
    .removeCoupons({ showErrorNotification: false })
    .then(() => {
      if (onSuccess) onSuccess();
    })
    .catch((err) => {
      cartErrorToast(err, "Couldn't remove coupon");
      if (btn) btn.disabled = false;
